...
riverLines_water_quality_NN_wgs84<-riverLines_water_quality__wgs84%>%select(HydroID,NextDownID,StreamOrde,measure,value,mesrmnt,geometry)
rm(riverLines)
...
```{r}
Marokopa_connected_ID
connected_river_segments <- riverLines[riverLines$HydroID %in% Marokopa_connected_ID, ]
# 找到StreamOrde最大的两个值
max_stream_orders <- sort(unique(connected_river_segments$StreamOrde), decreasing = TRUE)[1:3]

# 绘制线段，先绘制粗线段再绘制细线段
plot(st_geometry(connected_river_segments), col = "lightblue", lwd = 0.5, main = "Marokopa River")
